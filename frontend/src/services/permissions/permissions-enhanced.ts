 ...
 * Handles both direct backend names and natural language variations
 */
...
/**
 * Verify user role is valid and matches expected format
 * Returns true if role is properly assigned
 */
export function isRoleValid(role: unknown): boolean {
  return normalizeRole(role) !== null;
}

/**
 * Get the backend role name from frontend role
 * Useful for API calls that need backend role names
 */
export function getFrontendToBackendRole(frontendRole: RoleName): string {
  ...
  return mapping[frontendRole] ?? "EMPLOYEE";
}
